...
class ConnectorManager extends EventEmitter {
  ...
  status(name) {
    ...
    return {
      ...
      lastError: entry.lastError || adapterStatus.error || adapterStatus.lastError || null,
  ...
  observeAdapterStatus(name, status = {}) {
    ...
    if (status.connected || rawState === 'connected') { entry.attempt=0; return this.setState(entry,'CONNECTED'); }
  ...
  async connect(name) {
    ...
    if (!entry.enabled) { this.setState(entry,'DISABLED'); return {ok:false,disabled:true,status:this.status(name)}; }
    clearTimeout(entry.timer); entry.timer=null; this.setState(entry,entry.attempt?'RETRYING':'CONNECTING');
    try {
      const deadline = Date.now()+this.connectTimeoutMs;
      const call = typeof entry.adapter.connect==='function' ? entry.adapter.connect() : entry.adapter.start?.();
      await Promise.race([Promise.resolve(call),new Promise((_,reject)=>setTimeout(()=>reject(new Error(`Connector ${name}: Start-Timeout nach ${this.connectTimeoutMs} ms`)),this.connectTimeoutMs))]);
      ...
      entry.attempt=0; entry.connectedAt=new Date().toISOString(); this.setState(entry,'CONNECTED'); return {ok:true,status:this.status(name)};
    } catch (error) {
      entry.lastError=String(error.message||error); this.setState(entry,'ERROR',error);
  ...
  scheduleRetry(entry) {
    clearTimeout(entry.timer); const delay=BACKOFF_MS[Math.min(entry.attempt,BACKOFF_MS.length-1)]; entry.attempt+=1;
    this.setState(entry,'RETRYING',entry.lastError); entry.timer=setTimeout(()=>this.connect(entry.name),delay); entry.timer.unref?.(); this.emit('retry',{name:entry.name,delayMs:delay,attempt:entry.attempt});
  }

  async disconnect(name) {
    const entry=this.entries.get(name); if(!entry)return{ok:false,error:`Unbekannter Connector: ${name}`}; clearTimeout(entry.timer); entry.timer=null;
    try { if(typeof entry.adapter.disconnect==='function')await entry.adapter.disconnect(); else if(typeof entry.adapter.stop==='function')await entry.adapter.stop(); }
    catch(error){this.onLog?.('warn',`connector:${name}`,'STOP_FAILED',{message:error.message});}
    entry.attempt=0; entry.connectedAt=null; this.setState(entry,entry.enabled?'ERROR':'DISABLED'); return{ok:true,status:this.status(name)};
  }

  async healthCheck(name) {
    const entry=this.entries.get(name); if(!entry)return{ok:false,error:`Unbekannter Connector: ${name}`};
    if(typeof entry.adapter.healthCheck==='function'){try{return await entry.adapter.healthCheck();}catch(error){return{ok:false,error:error.message,status:this.status(name)}}}
    const status=this.status(name); return{ok:Boolean(status?.connected),status};
  }
